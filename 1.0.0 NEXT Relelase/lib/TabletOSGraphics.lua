...
function graphics.drawMenu()
	...
	local menu = core.settings.menu or {
		...
		{name = core.getLanguagePackages().OS_shutdown,contextMenu ={
			{name=core.getLanguagePackages().OS_reboot, callback = function() return core.pcall(dofile,"/bin/reboot.lua") end},
			{name=core.getLanguagePackages().OS_shutdown2, callback = function() return core.pcall(dofile,"/bin/shutdown.lua") end},
		}},
	}
...
function graphics.clickedToBarButton(x,y)
	...
	if x == forward then return "FORWARD" end
end
