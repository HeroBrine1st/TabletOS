...
local function errorReport(file,success,reason,...) --ПОIХАЛИ
	if success then return success,reason, ... end
	if type(reason) == "table" and reason.reason == "terminated" then return nil, "terminated" end --УХХХХХ КОСТЫЛИ
		...
		if xFile then
			if button == 0 then
				if not fs.isDirectory(xFile) then
					...
					errorReport(xFile,core.pcall(dofile,xFile))
