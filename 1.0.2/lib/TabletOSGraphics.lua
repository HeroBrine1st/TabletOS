...
function graphics.drawBars(nPO)
	...
	do
		...
		buffer.drawText(w-3,1,0xFFFFFFF,str)
		local fore = graphics.theme.bars.background
		--if network.isActive() then fore = 0x000000 end
		--if network.isConnected() then fore = 0xFFFFFF end
		buffer.set(w-5,1,graphics.theme.bars.background,fore,"N")
	end
end
...
-- function graphics.window(label,elements) --перерисовку содержимого под окно делайте в приложении, это значительно экономит память
-- 	local x,y,w,h = 1,1,1,1
-- 	local function widthCorrection(newW)
-- 		w = math.max(w,newW)
-- 	end
-- 	local scroll = 0
-- 	local commonH = 1
-- 	local function setScroll(motion)
-- 		local s = scroll + motion
-- 		if s >= 0 and s =< commonH-h then
-- 			scroll = s
-- 		end
-- 	end
-- 	for i = 1, #elements do
-- 		local h = objectsH[_type]
-- 		local element = elements[i]
-- 		local _type = element.type
-- 		if _type == "buttonGroup" then
-- 			local w = 1
-- 			for _,v in pairs(element.buttons) do
-- 				w = w + #v[3] + 1
-- 			end
-- 			widthCorrection(w)
-- 		elseif _type = "centerText" then
-- 			widthCorrection(#element.text + 2)
-- 		elseif _type = "textField" then
-- 			local lines = element.lines
-- 			for i = 1, #lines do
-- 				h = h + 1
-- 				widthCorrection(3+lines[i]:len())
-- 			end
-- 		elseif _type = "textInput" then
-- 			--ничего не делаем
-- 		end
-- 		commonH = commonH + h + 1
-- 	end
-- 	local function drawElement(Y,element)
-- 		local h = objectsH[_type]
-- 		local _type = element.type
-- 		if _type == "buttonGroup" then
-- 			buffer.drawRectangle(x,Y,w,h,0x0,0x0," ")
-- 			for i = 1, #element.buttons do
-- 				local button = element.buttons[i]
