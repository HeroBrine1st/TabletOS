...
function cornerMenuSettings(x,y)
	local menu = core.settings.menu or {
		{name = core.getLanguagePackages().OS_settings,file = "/TabletOS/Apps/Settings.lua"},
		{name = core.getLanguagePackages().OS_shutdown,contextMenu = {
			{name=core.getLanguagePackages().OS_reboot, file = "/bin/reboot.lua"},
			{name=core.getLanguagePackages().OS_shutdown2, file = "/bin/shutdown.lua"},
		}},
	}
	if type(menu) == "string" then menu = serial.unserialize(menu) end
...
program.mainMenu = {
	...
	{name=function() return core.getLanguagePackages().Settings_corner_menu end, onClick=function(x,y) cornerMenuSettings(x,y) end},
	{type="Separator"}, 
...
program.updates = {
	{name=function() return core.getLanguagePackages().Settings_installUpdates end, onClick=function() if updater.hasUpdate() then updater.update() end end,type="Button"},
