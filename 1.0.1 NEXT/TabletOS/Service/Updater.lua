...
if component.isAvailable("internet") then
	...
	error("SERVICE_CODE:01,REASON:NO_CONNECTION_TO_INTERNET")
end
...
local function request(uri,post,headers)
	...
	if success then
		...
		repeat
			...
			if reason then error("SERVICE_CODE:02,REASON:"..tostring(reason)) end
...
for i = 1, #new do
	if not new.exp then
		...
		local filels = serial.unserialize(request(new[i].raw))
		for i = 1, #filels do
...
local function download(url,path)
	...
	if not file then error("SERVICE_CODE:101,REASON:" .. tostring(reason)) end
		...
		if reqH then
			...
			if resData and resData["Content-Length"] then
				...
				while downloadedLength < contentLength do
					...
					if not data and reason then reqH.close() error("SERVICE_CODE:02,REASON:" .. tostring(reason)) end 
				...
				error("SERVICE_CODE:02,REASON:ERR_HEADER_ABSENT")
			end
		else
			error("SERVICE_CODE:02,REASON:ERR_NAME_NOT_RESOLVED")
		end
	else
		error(reqH)
	end
